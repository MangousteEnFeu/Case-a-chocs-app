...
import { 
  ...
  ChevronRight, Target, Search, Filter, X, Edit3, Upload, 
  SortAsc, SortDesc, ChevronDown, Save, Trash2, Eye, BarChart3,
...
// Jauge circulaire de remplissage
  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center gap-2 px-3 py-2 border border-gray-700 bg-black text-white text-xs font-mono hover:border-white transition-all min-w-[140px]"
      >
        {icon}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-[#111] border-2 border-white w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-[8px_8px_0px_0px_#E91E63]">
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              className="w-full bg-black border border-gray-700 text-white px-4 py-3 font-mono focus:border-[#E91E63] focus:outline-none resize-none"
            />
          </div>
          ...
          {editedEvent.petziExternalId && (
            <div className="p-4 border border-[#00FFFF] bg-[#00FFFF]/10">
              ...
              {editedEvent.lastSyncAt && (
                <div className="text-xs font-mono text-gray-500 mt-2">
                  Dernière sync: {new Date(editedEvent.lastSyncAt).toLocaleString('fr-CH')}
                </div>
              )}
            </div>
          )}
        ...
        {/* Footer */}
        <div className="sticky bottom-0 bg-[#111] border-t-2 border-white p-4 flex items-center justify-between gap-4">
          <button
            onClick={onClose}
            className="px-6 py-3 border border-gray-700 text-gray-500 font-mono text-sm uppercase hover:border-white hover:text-white transition-all"
          >
            Annuler
          </button>
          <div className="flex gap-4">
            <button
              onClick={handlePush}
              disabled={isPushing}
              className="flex items-center gap-2 px-6 py-3 bg-[#00FFFF] text-black font-mono text-sm uppercase hover:bg-[#00FFFF]/80 transition-all disabled:opacity-50"
            >
            ...
            <button
              onClick={handleSave}
              disabled={isSaving}
              className="flex items-center gap-2 px-6 py-3 bg-[#E91E63] text-white font-mono text-sm uppercase hover:bg-[#E91E63]/80 transition-all disabled:opacity-50"
            >
    ...
    <div className="border-2 border-white bg-[#111] p-4 space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        <button
          ...
          className={`flex items-center gap-2 px-3 py-2 border text-xs font-mono uppercase transition-all ${
...
// Tableau des événements (pour vue globale)
const EventsTable: React.FC<{
  ...
  return (
    <div className="border-2 border-white bg-[#111] overflow-hidden">
      {/* Table Header */}
      <div className="grid grid-cols-12 gap-4 p-4 border-b-2 border-white bg-black">
        ...
        {events.length === 0 ? (
          ...
          events.map((event) => {
            ...
            return (
              <div
                key={event.id}
                className="grid grid-cols-12 gap-4 p-4 border-b border-gray-800 hover:bg-black/50 transition-all group"
              >
                {/* Événement */}
...
const DashboardPage: React.FC = () => {
  ...
  // Charger les données pour la vue par événement
  useEffect(() => {
  ...
  // Charger les données pour la vue globale
  useEffect(() => {
  ...
  const handlePushToPetzi = async (event: HeedsEvent) => {
    ...
    // await api.syncEventToPetzi(event.id);
    ...
    console.log('Événement pushé vers PETZI:', event);
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-4xl font-['Anton'] text-white tracking-wider">
            TABLEAU DE BORD
          </h1>
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Event Selector (mode événement) */}
          {viewMode === 'event' && (
            <select
              value={selectedEventId}
              onChange={(e) => setSelectedEventId(e.target.value)}
              className="bg-black border-2 border-white text-white px-4 py-2 font-mono text-sm focus:border-[#E91E63] focus:outline-none max-w-[300px]"
            >
              {events.map(event => (
      ...
      {viewMode === 'event' && !loading && isEmptyDraft && (
        <div className="h-96 flex flex-col items-center justify-center gap-4 border-2 border-dashed border-gray-800">
          <AlertCircle size={60} className="text-gray-600" />
          <h2 className="text-2xl font-['Anton'] text-white">Événement en brouillon</h2>
          <p className="text-gray-500 font-mono text-sm text-center max-w-md">
            Cet événement n'a pas encore été synchronisé avec PETZI. 
            Aucune donnée de vente n'est disponible.
          </p>
          <button
            onClick={() => selectedEvent && setEditingEvent(selectedEvent)}
            className="mt-4 flex items-center gap-2 px-6 py-3 bg-[#E91E63] text-white font-mono text-sm uppercase"
          >
            <Edit3 size={16} />
            Modifier et synchroniser
          </button>
          <div className="flex gap-4 mt-4 font-mono text-xs text-gray-500 uppercase">
            <span>Billets vendus: 0</span>
            <span>•</span>
            <span>Revenus: CHF 0.-</span>
          </div>
        </div>
      )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <div className="border-2 border-white bg-[#111] p-0 relative shadow-[8px_8px_0px_0px_#222]">
                ...
                <div className="p-6 border-b-2 border-white flex justify-between items-center">
                  <h3 className="text-2xl text-white">ÉVOLUTION DES VENTES</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="border-2 border-white bg-[#111] p-6">
                  <h3 className="text-xl text-white mb-4 uppercase">Répartition par catégorie</h3>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        ...
                        <Tooltip 
                          ...
                          formatter={(value: number, name: string) => [`${value} billets`, name]}
                        />
                      </PieChart>
                    </ResponsiveContainer>
                  </div>
                ...
                {/* Geo Distribution */}
                <div className="border-2 border-white bg-[#111] p-6">
                  <h3 className="text-xl text-white mb-4 uppercase flex items-center gap-2">
            ...
            <div className="space-y-8">
              ...
              <div className="border-2 border-white bg-[#111] overflow-hidden relative shadow-[8px_8px_0px_0px_#222]">
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <span className={`text-[10px] font-mono px-2 py-1 border ${
                      selectedEvent?.status === 'SYNCED' ? 'text-[#00FF00] border-[#00FF00]' :
                      selectedEvent?.status === 'CONFIRMED' ? 'text-[#00FFFF] border-[#00FFFF]' :
                      'text-gray-500 border-gray-600'
                    }`}>
                      {selectedEvent?.status === 'SYNCED' ? 'Synchronisé' : selectedEvent?.status === 'CONFIRMED' ? 'Confirmé' : 'Brouillon'}
                    </span>
                  </div>
                  ...
                  <div className="space-y-3 text-sm font-mono">
                    <div className="flex items-center gap-3 text-gray-400">
                      ...
                      <span>{new Date(report.eventDate).toLocaleDateString('fr-CH', { 
                        weekday: 'long', 
                        day: 'numeric', 
                        month: 'long', 
                        year: 'numeric' 
                    ...
                    {selectedEvent?.genre && (
                      <div className="flex items-center gap-3 text-gray-400">
                        <Target size={16} className="text-[#00FF00]" />
                        <span className="bg-[#E91E63]/20 text-[#E91E63] px-2 py-0.5 text-xs">
                          {selectedEvent.genre}
                        </span>
                      </div>
                    )}
              ...
              <div className="border-2 border-white bg-[#111] p-6 flex flex-col items-center">
                <h3 className="text-xl text-white mb-4 uppercase w-full">Jauge de remplissage</h3>
          ...
          {filteredStats && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <StatCard 
                ...
                icon={Ticket}
                color="primary"
                subtext={`Taux: ${((filteredStats.totalSold / filteredStats.totalCapacity) * 100).toFixed(1)}%`}
              />
              <StatCard 
                title="Événements" 
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="border-2 border-white bg-[#111] p-0 relative shadow-[8px_8px_0px_0px_#222]">
              ...
              <div className="p-6 border-b-2 border-white">
                <h3 className="text-2xl text-white">ÉVOLUTION GLOBALE DES VENTES</h3>
              </div>
              <div className="h-[300px] w-full p-4">
                <ResponsiveContainer width="100%" height="100%">
      ...
      {viewMode === 'global' && loading && (
        <div className="h-96 flex flex-col items-center justify-center gap-4 border-2 border-dashed border-gray-800">
          ...
          <span className="font-mono text-[#00FFFF] uppercase animate-pulse">
            Agrégation des données de tous les événements...
          </span>
          <span className="font-mono text-gray-500 text-xs">
            {events.length} événements à analyser
          </span>
        </div>
      )}
